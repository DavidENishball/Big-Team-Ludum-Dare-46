Shader "Glass/GlassTransparent" {
    Properties {
        ...
        _Amplify("Amplify", Range(0.0, 1.0)) = 1.0
        _Position("Position", Vector) = (1, 0, 0, 0)
        _Hotspot("Hot Spot", Vector) = (1, 0, 0, 0)
        _HotspotRange("Hotspot range", Float) = 0.5
    }
    SubShader {
        Tags {
            "Queue"="Transparent"
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                //Glitch effect
                float amp = _Amplify * abs(tan(min(_Time.x + (v.vertex.y / 5), 1.0) * 10));
                amp = clamp(amp, -1.0, 1.0);
                float intensity = sin(_Time.x * 40);
                amp *= intensity;
                amp *= lerp(0, 1, distance(float3(_Hotspot.x, _Hotspot.y, 0), float3(_Position.x, _Position.y, 0)) < float3(_HotspotRange, _HotspotRange, _HotspotRange));

                v.vertex.xyz += float4(sin(rand(float3(_Position.y + _Time.x, _Position.z + _Time.x, _Position.y + _Time.x))) * amp,
                  0,
                  0, 0);
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.posWorld.xyz += float4(sin(rand(float3(_Position.y + _Time.x, _Position.z + _Time.x, _Position.y + _Time.x))) * amp,
                  0,
                  0, 0);
